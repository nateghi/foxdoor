...
+ (instancetype)emptyItem {
    ...
    dispatch_once(&onceToken, ^{
        emptyItem = [[RNGridMenuItem alloc] initWithView:nil title:nil action:nil];
    });

    return emptyItem;
}

- (instancetype)initWithView:(UIView *)view title:(NSString *)title action:(dispatch_block_t)action {
    if ((self = [super init])) {
        _view = view;
        _title = [title copy];
        _action = [action copy];
...
- (instancetype)initWithView:(UIView *)view title:(NSString *)title {
    return [self initWithView:view title:title action:nil];
}

- (instancetype)initWithView:(UIView *)view {
    return [self initWithView:view title:nil action:nil];
}

- (instancetype)initWithTitle:(NSString *)title {
    return [self initWithView:nil title:title action:nil];
}
...
- (instancetype)initWithItems:(NSArray *)items {
    if ((self = [super init])) {
        _itemSize = CGSizeMake(300.f, 90.f);
        _cornerRadius = 8.f;
...
- (instancetype)initWithViews:(NSArray *)views {
    ...
    for (UIView *view in views) {
        RNGridMenuItem *item = [[RNGridMenuItem alloc] initWithView:view];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (self.selectedItemView != nil) {
        RNGridMenuItem *item = self.items[self.selectedItemView.tag];
   ...
   willDismissWithSelectedItem:item
                       atIndex:self.selectedItemView.tag];
        }
...
//        [self.menuView addSubview:item.view];
//        RNMenuItemView *itemView = [[RNMenuItemView alloc] init];

//        itemView = (RNMenuItemView*)item;
        [self.menuView addSubview:item.view];
        [self.itemViews addObject:item.view];
    }];
...
- (void)selectItemViewAtPoint:(CGPoint)point {
    ...
    RNGridMenuItem *item = self.items[selectedItemView.tag];
    ...
    NSLog(@"the view # %d", self.selectedItemView.tag);
}
