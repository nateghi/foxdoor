...
- (NSArray*)boxes;
- (UIView*)headerViewForTextField:(UITextField*)textField :(NSString*)statusStraing;
...
- (void)loadView
{
    ...
    if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad && [[UIScreen mainScreen] bounds].size.height > 480) {
        ...
        tableViewFrame.origin.y += 44.0;
            ...
            _enterPasscodeTableView.frame = CGRectMake(_enterPasscodeTableView.frame.origin.x, _enterPasscodeTableView.frame.origin.y + 44, _enterPasscodeTableView.frame.size.width, _enterPasscodeTableView.frame.size.height);
    ...
    // Fox Door
    if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad && [[UIScreen mainScreen] bounds].size.height < 500){
        
    _enterPasscodeTableView.frame = CGRectMake(_enterPasscodeTableView.frame.origin.x, _enterPasscodeTableView.frame.origin.y , _enterPasscodeTableView.frame.size.width, _enterPasscodeTableView.frame.size.height);
    }
	...
	_setPasscodeTableView = [[UITableView alloc] initWithFrame:_enterPasscodeTableView.frame style:UITableViewStyleGrouped];
...
//    CGRect viewBounds = [_confirmPasscodeTableView bounds];
//    viewBounds.origin.y = 18;
//    viewBounds.size.height = viewBounds.size.height + 88;
//    _confirmPasscodeTableView.frame = viewBounds;
...
//        self.navigationItem.title = KKPasscodeLockLocalizedString(@"Set Passcode", @"");
        statusString = @"Set Passcode";
        self.navigationItem.title = KKPasscodeLockLocalizedString(@"", @"");
...
//        self.navigationItem.title = KKPasscodeLockLocalizedString(@"Turn off Passcode", @"");
        statusString = @"Turn off Passcode";
        self.navigationItem.title = KKPasscodeLockLocalizedString(@"", @"");
...
//        self.navigationItem.title = KKPasscodeLockLocalizedString(@"Enter Passcode", @"");
	...
	if (_mode == KKPasscodeModeSet || _mode == KKPasscodeModeChange) {
		if (_passcodeLockOn) {
			_enterPasscodeTableView.tableHeaderView = [self headerViewForTextField:_enterPasscodeTextField :statusString];
		...
		_setPasscodeTableView.tableHeaderView = [self headerViewForTextField:_setPasscodeTextField :statusString];
        
		[_tableViews addObject:_setPasscodeTableView];
		[_textFields addObject:_setPasscodeTextField];
		[_boxes addObject:[self boxes]];
		UIView *boxesView = [[UIView alloc] initWithFrame:CGRectMake((self.view.bounds.size.width - totalBoxesWidth) * 0.5, 30, totalBoxesWidth, kPasscodeBoxHeight)];
		boxesView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
		for (int i = 0; i < [[_boxes lastObject] count]; i++) {
			[boxesView addSubview:[[_boxes lastObject] objectAtIndex:i]];
		}
		...
		_confirmPasscodeTableView.tableHeaderView = [self headerViewForTextField:_confirmPasscodeTextField :statusString];
		[_tableViews addObject:_confirmPasscodeTableView];
		[_textFields addObject:_confirmPasscodeTextField];
		[_boxes addObject:[self boxes]];
		UIView *boxesConfirmView = [[UIView alloc] initWithFrame:CGRectMake((self.view.bounds.size.width - totalBoxesWidth) * 0.5, 30, totalBoxesWidth, kPasscodeBoxHeight)];
...
//    _failedAttemptsLabel.frame = _failedAttemptsView.frame = CGRectMake((self.view.bounds.size.width - (size.width + (self.isSmallLandscape ? 20.0f : 40.0f))) / 2, self.isSmallLandscape ? 75.0f : 150.0f, size.width + (self.isSmallLandscape ? 20.0f : 40.0f), size.height + (self.isSmallLandscape ? 5.0f : 10.0f));
	...
	_failedAttemptsLabel.frame = _failedAttemptsView.frame = CGRectMake((self.view.bounds.size.width - (size.width + (self.isSmallLandscape ? 20.0f : 40.0f))) / 2, self.isSmallLandscape ? 95.0f : 170.0f, size.width + (self.isSmallLandscape ? 20.0f : 40.0f), size.height + (self.isSmallLandscape ? 5.0f : 10.0f));
...
- (void)moveToNextTableView
{
    ...
    // Fox Door newTableView.frame = self.view.frame;
        if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad && [[UIScreen mainScreen] bounds].size.height > 480) {
            
    newTableView.frame = CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y - 20 , self.view.frame.size.width, self.view.frame.size.height);
        } else {
            
                newTableView.frame = CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y - 62 , self.view.frame.size.width, self.view.frame.size.height);
        }
...
- (void)moveToPreviousTableView
{
	...
	[UIView beginAnimations:@"" context:nil];
	[UIView setAnimationDuration:0.25];
	oldTableView.frame = CGRectMake(oldTableView.frame.origin.x + self.view.bounds.size.width, oldTableView.frame.origin.y + 44, oldTableView.frame.size.width, oldTableView.frame.size.height);
	newTableView.frame = CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y + 44, self.view.frame.size.width, self.view.frame.size.height);
	[UIView commitAnimations];
		...
		if (_mode == KKPasscodeModeSet) {
			...
			if ([textField isEqual:_enterPasscodeTextField]) {
				...
				if ([passcode isEqualToString:_setPasscodeTextField.text]) {
					...
					_passcodeConfirmationWarningLabel.frame = CGRectMake(0.0, 132.0, self.view.bounds.size.width, 30.0);
					[self moveToNextTableView];
				}
			} else if ([textField isEqual:_confirmPasscodeTextField]) {
				if (![_confirmPasscodeTextField.text isEqualToString:_setPasscodeTextField.text]) {
...
- (UIView*)headerViewForTextField:(UITextField*)textField :(NSString*)statusStraing
{
	...
	UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 4.0, self.view.bounds.size.width, 70.0)];
    UILabel *statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, self.isSmallLandscape ? 14.0f : 28.0f, self.view.bounds.size.width, 30.0)];
    statusLabel.text = statusStraing;
    statusLabel.textAlignment = NSTextAlignmentCenter;
    
	UILabel *headerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, self.isSmallLandscape ? 14.0f : 58.0f, self.view.bounds.size.width, 30.0)];
    ...
    statusLabel.font = [UIFont boldSystemFontOfSize:self.isSmallLandscape ? 18.0f : 20.0f];
	statusLabel.shadowOffset = CGSizeMake(0, 1.0);
	statusLabel.shadowColor = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1.0];
    
	headerLabel.font = [UIFont boldSystemFontOfSize:self.isSmallLandscape ? 15.0f : 17.0f];
	headerLabel.shadowOffset = CGSizeMake(0, 1.0);
	headerLabel.shadowColor = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1.0];
	
	if ([textField isEqual:_setPasscodeTextField]) {
		_passcodeConfirmationWarningLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, self.isSmallLandscape ? 103.0f : 166.0, self.view.bounds.size.width, 30.0)];
